...
| Export | Description |
| --- | --- |
| `useStorage` | Access the key-value [storage layer](/docs/storage), optionally scoped to a base (e.g. `useStorage("data")`). |
...
| Export | Description |
| --- | --- |
| `useDatabase` | Access a configured SQL [database](/docs/database) connection (default: `useDatabase()`, named: `useDatabase("users")`). |
...
| `getRouteRules` | Get the resolved [route rules](/docs/routing#route-rules) for a method and pathname: `getRouteRules("GET", "/blog/post")`. |

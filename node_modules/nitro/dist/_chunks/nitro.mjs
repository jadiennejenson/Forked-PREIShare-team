...
import { _, build, compressPublicAssets, ensureDep, ensureLibDeps, isDepInstalled, join, prettyPath, relative, resolve, resolveConnectorDeps, resolveDatabaseConnections, resolveDriverDeps, resolveModulePath, resolveModuleURL, resolveNitroPath, resolveStorageMounts, runParallel, scanUnprefixedPublicAssets, src_default, v, writeFile as writeFile$1 } from "../_build/common.mjs";
...
import { ie } from "../_libs/get-tsconfig+resolve-pkg-maps.mjs";
import { findWorkspaceDir } from "../_libs/pkg-types.mjs";
import { scanAndSyncOptions, scanHandlers } from "./nitro2.mjs";
import { initNitroRouting } from "./nitro3.mjs";
import "../_dev.mjs";
import { parse, walk } from "../_libs/ultrahtml.mjs";
...
async function resolveAssetsOptions(options) {
	...
	for (const asset of options.publicAssets) {
		...
		asset.maxAge = (routeRule?.cache)?.maxAge ?? asset.maxAge;
...
async function resolveDatabaseOptions(options) {
	if (!options.experimental.database) return;
	...
	await ensureLibDeps(resolveDatabaseConnections(options).map((connection) => ({
...
async function resolveTsconfig(options) {
	...
	if (!options.typescript.tsConfig) options.typescript.tsConfig = loadTsconfig(root);
}
function loadTsconfig(root) {
	const tsConfigPath = join(root, "tsconfig.json");
	let tsconfig;
	try {
		tsconfig = ie(tsConfigPath, tsconfigCache);
	} catch {
		return {};
	}
	tsconfig.compilerOptions ??= {};
	if (!tsconfig.compilerOptions.baseUrl) tsconfig.compilerOptions.baseUrl = resolve(tsConfigPath, "..");
	return tsconfig;
}
...
async function resolveRouteRulesOptions(options) {
	options.routeRules = normalizeRouteRules$1(options);
}
function normalizeRouteRules$1(config) {
	const routeRules = config.routeRules || {};
	for (const route in routeRules) if (routeRules[route] && "basicAuth" in routeRules[route]) throw new Error(`[nitro] \`basicAuth\` is not a route rule (\`${route}\`). Register h3's \`basicAuth\` middleware instead: globally from \`middleware/\`, or per route with \`defineHandler({ middleware: [basicAuth({ ... })], handler })\`.`);
	return normalizeRouteRules(routeRules);
}
...
async function resolveStorageOptions(options) {
	await ensureLibDeps(resolveStorageMounts(options).map((mount) => ({
		name: mount.name,
...
async function resolveURLOptions(options) {
	...
	if (options.buildAssetsDir !== void 0) options.buildAssetsDir = normalizeBuildAssetsDir(options.buildAssetsDir) || void 0;
}
...
async function resolveBuilder(options) {
	...
	if (options.builder) {
		...
		if (pkg !== "rolldown") {
			...
			})) throw new Error(`Nitro builder package "${pkg}" is not installed. Please install it in your project dependencies.`);
		}
		return;
	}
...
async function updateNitroConfig(nitro, config) {
	nitro.options.routeRules = normalizeRouteRules$1(config.routeRules ? config : nitro.options);
...
const installedModules = /* @__PURE__ */ new WeakMap();
async function installModules(nitro, moduleInputs) {
...
async function extractLinks(html, from, res, crawlLinks) {
	const links = [];
	const _links = [];
	if (crawlLinks) await walk(parse(html), (node) => {
	...
	for (const link of links) {
		...
		else linkParents.set(link, /* @__PURE__ */ new Set([from]));
	}
	return links;
}
...
async function prerender(nitro) {
	...
	if (routes.size === 0) {
		if (nitro.options.prerender.crawlLinks) routes.add("/");
		else return;
	}
...
async function _getTasksContext(opts) {
	...
	const devFetch = (path, options) => {
		return new Promise((resolve, reject) => {
			...
			const request = httpNative.request(url, {
				socketPath,
...
export { createNitro, installModules, listTasks, loadOptions, prerender, runTask };

import { __commonJSMin, __require, __toESM } from "../_common.mjs";
import { createFilter } from "../_build/common.mjs";
import Ie, { realpathSync } from "fs";
import Re, { dirname, extname, normalize, resolve, sep } from "path";
...
var require_async = /* @__PURE__ */ __commonJSMin(((exports, module) => {
	...
	module.exports = function resolve(x, options, callback) {
		...
		maybeRealpath(realpath, path$1.resolve(basedir), opts, function(err, realStart) {
			if (err) cb(err);
			else init(realStart);
		});
		var res;
			...
			function load(exts, x, loadPackage) {
				...
				function onpkg(err, pkg_, dir) {
					...
					if (dir && pkg && opts.pathFilter) {
						...
						if (r) return load([""].concat(extensions), path$1.resolve(dir, r), pkg);
					}
					isFile(file, onex);
				}
					...
					readPackage(readFile, pkgfile, function(err, pkgParam) {
						...
						if (pkg && pkg.main) {
							...
							loadAsFile(path$1.resolve(x, pkg.main), pkg, function(err, m, pkg) {
								...
								loadAsDirectory(path$1.resolve(x, pkg.main), pkg, function(err, n, pkg) {
									if (err) return cb(err);
...
promisify(Ie.access);
const readFile$1 = promisify(Ie.readFile);
const realpath$1 = promisify(Ie.realpath);
const stat$1 = promisify(Ie.stat);
...
function getPackageInfo(options) {
	...
	if (!ignoreSideEffectsForRoot || rootDir !== pkgRoot) {
		...
		if (typeof packageSideEffects === "boolean") internalPackageInfo.hasModuleSideEffects = () => packageSideEffects;
			...
			internalPackageInfo.hasModuleSideEffects = createFilter(finalPackageSideEffects, null, { resolve: pkgRoot });
		}
	}
...
async function findPackageJson(base, moduleDirs) {
	const { root } = Re.parse(base);
	let current = base;
	while (current !== root && !isModuleDir(current, moduleDirs)) {
		const pkgJsonPath = Re.join(current, "package.json");
		if (await fileExists(pkgJsonPath)) {
			const pkgJsonString = Ie.readFileSync(pkgJsonPath, "utf-8");
			return {
...
const readFile = promisify(Ie.readFile);
...
function nodeResolve(opts = {}) {
	...
	const resolveLikeNode = async (context, importee, importer, custom) => {
		...
		if (options.modulesOnly && await fileExists(location)) {
			...
			if ((0, import_is_module.default)(code)) return {

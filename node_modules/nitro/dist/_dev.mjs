import { dirname, extname, join, m, resolve, src_default, v, writeDevBuildInfo } from "./_build/common.mjs";
...
function isLocalDevRequest(event) {
	...
	const isUnixSocket = !socket?.remoteAddress && !socket?.localAddress && Object.keys(socket?.address?.() || {}).length === 0 && socket?.readable && socket?.writable && !socket?.remotePort;
...
function createVFSHandler(nitro) {
	return defineHandler(async (event) => {
		if (!isLocalDevRequest(event)) throw new HTTPError({
			statusText: "Forbidden IP",
			status: 403
		});
...
async function defaultHandler(error, event, opts) {
	...
	if (unhandled && !opts?.silent) {
		...
		consola$1.error(`[request error] [${event.req.method}] ${url}\n\n`, ansiError);
	}
...
async function loadStackTrace(error) {
	...
	let parsed;
	try {
		parsed = await new ErrorParser().defineSourceLoader((frame) => sourceLoader(frame, consumerCache)).parse(error);
	} finally {
		for (const consumer of consumerCache.values()) await consumer.then((c) => c?.destroy()).catch(() => {});
	}
	const stack = error.message + "\n" + parsed.frames.map((frame) => fmtFrame(frame)).join("\n");
	Object.defineProperty(error, "stack", { value: stack });
	if (error.cause) await loadStackTrace(error.cause).catch(consola$1.error);
}
async function loadConsumer(fileName, cache) {
...
async function sourceLoader(frame, consumerCache) {
	...
	#createApp(catchAllHandler) {
		...
		for (const h of this.nitro.options.devHandlers) {
			...
			if (h.middleware || !h.route) {
				if (h.route) app.use(h.route, handler, { method: h.method });
		...
		const assertLocalTaskRequest = (event) => {
			if (!isLocalDevRequest(event)) throw new HTTPError({
				statusText: "Forbidden IP",
				status: 403
			});
...
var NitroDevServer = class NitroDevServer extends NitroDevApp {
	...
	async #reload() {
		await this.#shutdownWorker();
		const runnerName = this.nitro.options.devServer.runner || process.env.NITRO_DEV_RUNNER || "node-worker";
		...
		try {
			await new Promise((resolve) => {
				...
				const listener = (message) => {
					if (message?.event === "exit") done();
				};
				this.#manager.onMessage(listener);
				try {
					this.#manager.sendMessage({ event: "shutdown" });
				} catch (error) {
					this.nitro.logger.warn(`Could not send shutdown message to the dev worker: ${error instanceof Error ? error.message : String(error)}`);
					done();
				}
			});
		} finally {

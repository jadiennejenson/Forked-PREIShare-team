import { getBuildInfo, relative, resolve } from "../../_build/common.mjs";
import { defineCommand } from "../../_libs/citty.mjs";
import { buildArgs, build_default } from "./build.mjs";
...
var deploy_default = defineCommand({
	...
	async run(ctx) {
		...
		const rootDir = resolve(ctx.args.dir || ctx.args._dir || ".");
		const { buildInfo, outputDir } = await getBuildInfo(rootDir);
		if (!buildInfo) {
			consola$1.error("No build info found, cannot deploy.");
			process.exit(1);
		}
		if (!buildInfo.commands?.deploy) {
			consola$1.error(`The \`${buildInfo.preset}\` preset does not have a default deploy command.\n\nTry using a different preset with the \`--preset\` option, or configure a deploy command in the Nitro config, or deploy manually.`);
			process.exit(1);
		}

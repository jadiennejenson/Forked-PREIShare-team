...
import { resolveRuntimeDep } from "./common-runtime-deps.mjs";
...
async function loadWranglerConfig(opt, env, entryPath, wranglerModule) {
	if (!opt) return;
	...
	const wrangler = await resolveRuntimeDep({
		name: "wrangler",
		option: "wranglerModule",
		value: wranglerModule
	});
	if (!wrangler?.unstable_readConfig || !wrangler.unstable_getMiniflareWorkerOptions) return mergeWranglerMiniflareOptions(configPath ? readWranglerConfigMinimal(configPath, env) : void 0, inline ? mapWranglerConfigToMiniflare(applyWranglerEnv(inline, env)) : void 0);
	try {
...
var MiniflareEnvRunner = class extends BaseEnvRunner {
	...
	async #resolveMiniflare() {
		this.#miniflareModule = await resolveRuntimeDep({
			name: "miniflare",
			option: "miniflare",
	...
	async #initAsync() {
		...
		const wranglerOptions = await loadWranglerConfig(this.#wrangler, this.#wranglerEnv, entryPath, this.#wranglerModule);
			...
			if (!options.unsafeModuleFallbackService) {
				...
				options.unsafeModuleFallbackService = async (request) => {
					...
					if (fileUrlRaw.startsWith("file://")) try {
						...
						const referrerKey = referrer.startsWith("/") ? referrer.slice(1) : referrer;
						const referrerReal = modulePathMap.get(referrerKey) || (referrer.startsWith("/") ? referrer : "/" + referrer);
